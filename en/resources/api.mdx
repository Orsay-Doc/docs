...
Pushes a lead into a sequence, exactly like the Zapier or Make "Create Lead" action, but with a plain API key. The organization is derived from the key: no OAuth token to refresh, so no connection that can expire mid-scenario.
...
Only `first_name` and `phone` are required. The phone number must be in international format. If `sequence_id` is omitted, the lead is created without being subscribed to a sequence.
...
<Note>
The target sequence must use the **Contact subscribed to sequence** trigger. A `402` means the organization has no active subscription.
</Note>
